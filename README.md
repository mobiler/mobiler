...
- **Media & feedback** — images, avatars, progress bars, shimmer skeletons, swipe actions,
  ...
  **web view** (`WebView` — WKWebView / Android WebView / `<iframe>`; hosts any page or a hosted
  player embed like Bunny.net, with a `mobiler_core::bunny` URL helper), and an interactive
  **map** (`Map` — iOS MapKit / Android MapLibre / web MapLibre-GL, **no API key**; app-driven
  center/zoom + markers, with map/marker taps reported back via `input`).
...
- **Accessibility** — wrap any widget with `a11y(child, label)` (+ `with_a11y_hint` / `with_a11y_role`)
  to name it for a screen reader (VoiceOver / TalkBack): iOS `accessibilityLabel`/traits, Android
  `contentDescription`/role, web `aria-label`/role. Text honors the OS font-size setting on all three.
...
|---|---|---|
| HTTP | `cx.get / cx.post / cx.put / cx.patch / cx.delete / cx.request (builder) / cx.upload / cx.download (streaming)` | headers + byte body/response; streaming upload/download with progress + cancel (native shells land in CLI 0.49); multipart upload via cx.upload().multipart() (web today; native in CLI 0.50) |
...
| 📐 **sensors** | accelerometer / gyroscope → `x,y,z` | `mobiler plugin add sensors` |
...
_Recently shipped: **card long-press** (`with_long_press(card, E)` — a press-and-hold action on any
